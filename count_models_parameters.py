...
from src.original_segnet_stan import SegNet as OriginalSegNetSTAN
...
parser.add_argument('--task', default='semantic', type=str, help='task to train: all, semantic, depth, normal')
...
model2 = SegNetMTAN(opt.task).to(device)
model3 = SegNet(opt.task).to(device)
model4 = OriginalSegNetMTAN().to(device)  # Use the original SegNetMTAN for comparison
model5 = OriginalSegNetSTAN().to(device)  # Use the original SegNetSTAN for comparison
...
print('Parameter Space: ABS: {:.1f}, REL: {:.4f}'.format(count_parameters(model4),
                                                         count_parameters(model4) / 24981069))
print('Parameter Space: ABS: {:.1f}, REL: {:.4f}'.format(count_parameters(model5),
                                                         count_parameters(model5) / 24981069))